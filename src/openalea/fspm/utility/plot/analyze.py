...
def analyze_data(scenarios, outputs_dirpath, inputs_dirpath, target_folder_key=None, on_sums=False, on_raw_logs=False, animate_raw_logs=False, on_soil_logs=False, on_shoot_logs=False, on_performance=False, on_images=False, on_mtg=False,
        ...
        for scenario in scenarios:
            ...
            for sums_folder in sums_folders:    
                ...
                fig, _ = plot_csv(csv_dirpath=sums_folder,
                    csv_name="plant_scale_properties.csv", properties=["diffusion_AA_phloem", "import_Nm", "AA_catabolism", "AA_synthesis"], stacked=True, ignore_firsts=False, xlim=None, ylim=[1e-13, 2e-9], logscale=True, 
                    custom_suffix="_N_inputs", figsize=figsize, title="N inputs and transformations in symplasm (mol.plant-1.s-1)")
                fig, _ = plot_csv(csv_dirpath=sums_folder,
                    csv_name="plant_scale_properties.csv", properties=["diffusion_AA_soil", "export_AA","amino_acids_consumption_by_growth", "export_Nm", "diffusion_Nm_soil"], stacked=True, ignore_firsts=False, xlim=None, ylim=[1e-13, 2e-9], logscale=True, 
    ...
    if animate_raw_logs:
        ...
        if first_loop:
            ...
            dataset["Length-wise N exudation"] = Indicators.compute(d=dataset, formula="(1.4*(diffusion_AA_soil + apoplastic_AA_soil_xylem) + diffusion_Nm_soil + apoplastic_Nm_soil_xylem - import_AA) / length")
            ...
            for scenario in dataset.scenario.values:
                ...
                if running:
                    ...
                    if running or all_true:
                        ...
                        WB.XY_totals_all_times_plus_shoot(shoot_outputs=shoot_outputs, dataset=scenario_dataset, xshoot=Total_Photosynthesis, y="Raw_rhizodeposition", to_xunit="µmol/day", to_yunit="µmol/day", outputs_dirpath=os.path.join(outputs_dirpath, scenario, subscenario, "MTG_properties"), massic=True, custom_suffix="norm_surf")
                        WB.XY_totals_all_times_plus_shoot(shoot_outputs=shoot_outputs, dataset=scenario_dataset, xshoot=Total_Photosynthesis, y="Net_mineral_N_uptake", to_xunit="µmol/day", to_yunit="µmol/day", outputs_dirpath=os.path.join(outputs_dirpath, scenario, subscenario, "MTG_properties"), massic=True, custom_suffix="norm_surf")
                    ...
                    if running or all_true:
                        ...
                        WB.along_dist_from_tip_comp(dataset, variables=["Length_wise_raw_rhizodeposition", "Length-wise mineral N uptake"], 
                                                    ...
                                                    target_times=scenario_times, averaging_window=2,
    ...
    if on_images:
        outputdir = os.path.join(outputs_dirpath, scenarios[0], target_folder_key, 'root_images')
    ...
    if on_mtg:
        ...
        # unit = ["mol.s-1.m-1"] * 2
        plotted_properties = ["radial_import_water_xylem", "diffusion_AA_soil"]
        unit = ["m3.s-1.m-1", "mol.s-1.m-1"]
...
class WB:
    ...
    def scatter_plots(scenario_datasets, outputs_path, name_suffix="", xlog = False, c=None, discrete=False):
        massic = False
        scatter = True
        ...
        #                                         discrete=discrete, s=s, xlog=xlog, name_suffix=name_suffix, to_xunit="cm", xlim=xlim, to_yunit="nmol/(cm.h)", ylim=ylim, figsize=figsize, show_correlation=correlations)
        # fig, ax = XarrayPlotting.scatter_xarray(scenario_datasets, outputs_dirpath=outputs_path, x="distance_from_tip", y="Length-wise_radial_import_water", c=c, 
        #                                         discrete=discrete, s=s, xlog=xlog, name_suffix=name_suffix, to_xunit="cm", xlim=xlim, to_yunit="µL/(cm.h)", ylim=ylim, figsize=figsize, show_correlation=correlations)
        # fig, ax = XarrayPlotting.scatter_xarray(scenario_datasets, outputs_dirpath=outputs_path, x="distance_from_tip", y="Length_wise_raw_rhizodeposition", c=c, 
        #                                         discrete=discrete, s=s, xlog=xlog, name_suffix=name_suffix, to_xunit="cm", xlim=xlim, to_yunit="nmol/(cm.h)", ylim=ylim, figsize=figsize, show_correlation=correlations)
        
        # fig, ax = XarrayPlotting.scatter_xarray(scenario_datasets, outputs_dirpath=outputs_path, x="distance_from_tip", y="Lengthy_active_Ni_uptake", c=c, 
        #                                         discrete=discrete, s=s, xlog=xlog, name_suffix=name_suffix, to_xunit="cm", xlim=xlim, to_yunit="nmol/(cm.h)", ylim=ylim, figsize=figsize, show_correlation=correlations)
    ...
    def open_shoot_outputs(outputs_dirpath="", meteo_data_dirpath="", soil_data_dirpath="", scenario="", target_folder_key=None, shoot_outputs_dirpath=None, only_MS=True):
        ...
        soil_meteo = pd.read_csv(soil_data_dirpath, index_col='t')
        # Conversion step from hours 
        soil_meteo['Date'] = pd.to_datetime(soil_meteo.index.values*1e9*3600 + int(initial_date.timestamp())*1e9)
        soil_meteo['Date'] = soil_meteo["Date"].dt.strftime('%d/%m/%Y')
        
        # Path reference
        reference_postprocessing_dirpath = shoot_outputs_dirpath
    ...
    def plant_C_balance(shoot_outputs, dataset, outputs_dirpath, thermal_time = True, massic=False):
        ...
        net_hexose_exudation_C = Indicators.compute(d=dataset, formula="hexose_exudation - hexose_uptake_from_soil + phloem_hexose_exudation - hexose_uptake_from_soil").sum(dim="vid") * 1e6 * 3600 * 6
        ...
        if not massic:
            total_photo = Total_Photosynthesis.sum()
            total_unload = (Unloading_Sucrose_tot_C + Unloading_Amino_Acids_tot_C).sum() / total_photo
            total_reexport = Export_Amino_Acids_C.sum()  / total_photo
            total_net_unload = (Unloading_Sucrose_tot_C + Unloading_Amino_Acids_tot_C - Export_Amino_Acids_C).sum()  / total_photo
            prop_reexported = Export_Amino_Acids_C.sum() / (Unloading_Sucrose_tot_C + Unloading_Amino_Acids_tot_C).sum()
            tot_net_hexose_exudation_C = net_hexose_exudation_C.sum() / total_photo
            tot_net_AA_exudation_C = net_AA_exudation_C.sum() / total_photo
            tot_cells_release_C = cells_release_C.sum() / total_photo
            tot_mucilage_secretion_C = mucilage_secretion_C.sum() / total_photo
            tot_growth_respiration_C=growth_respiration_C.sum() / total_photo
            tot_maintenance_respiration_C = maintenance_respiration_C.sum() / total_photo
            tot_N_metabolic_respiration_C = N_metabolic_respiration_C.sum() / total_photo
            tot_Shoot_respiration = Shoot_respiration.sum() / total_photo

            print(dict(
                unloading = total_unload,
                rexport = total_reexport,
                prop_reexport = prop_reexported,
                net_unload = total_net_unload,
                tot_RD = tot_net_hexose_exudation_C + tot_net_AA_exudation_C + tot_cells_release_C + tot_mucilage_secretion_C,
                root_respi = tot_growth_respiration_C + tot_maintenance_respiration_C + tot_N_metabolic_respiration_C,
    ...
    def plant_C_balance_summary(shoot_outputs, outputs_dirpath, thermal_time = True, massic=False, balance=True, p_input=True):
        ...
        daily_shoot_struct_mass_produced_C = df_axe.groupby(['day'])['shoot_struct_mass_produced'].agg('sum').to_numpy() * conversion * 1e6 * 0.44 / 12.01
        
        df_axe["labile_C_shoot_hourly"] = 1e6 * (df_axe["sum_C_g_shoot"] - df_axe["mstruct_shoot"] * 0.44) / 12.01
        daily_labile_C_shoot = df_axe.groupby(['day'])['labile_C_shoot_hourly'].agg('mean').to_numpy() * conversion

        if thermal_time:
            # --- Thermal time (anchored + positional) ---
            df_meteo = shoot_outputs["meteo"].copy()

            # Ensure hourly, contiguous, and 0-based time
            df_meteo = df_meteo.reset_index(drop=True)
            df_meteo["t"] = np.arange(len(df_meteo), dtype=float)          # hours from 0

            # Degree-hours (base 0), then shift so TT(0h)=0 exactly
            deg_h = np.clip(df_meteo["air_temperature"].to_numpy(dtype=float), 0.0, None)
            tt_hourly = np.cumsum(deg_h, dtype=float)
            tt_hourly = np.concatenate(([0.0], tt_hourly[:-1])) / 24.0     # shift by 1 hour, to days

            days_hourly = df_meteo["t"].to_numpy(dtype=float) / 24.0       # 0, 1/24, 2/24, ...

            days = np.sort(df_roots['day'].unique().astype(int))     # 1,2,3,...
            idx_start = days * 24                               # hour index at day start
            idx_start = np.clip(idx_start, 0, len(tt_hourly) - 1)     # safety
            time_scale = tt_hourly[idx_start]     
    ...
    def root_symplasm_AA_fluxes(dataset, outputs_dirpath):
        ...
        for k, v in enumerate(variables):
            ax.plot(t, dataset[v].sum(dim="vid"), c=list(twenty_palette.values())[k], label=v)
        ax.legend()
    ...
    def environmental_conditions(shoot_outputs, dataset, outputs_dirpath):
        ...
        df_meteo_shoot = shoot_outputs["meteo"]
        df_meteo_soil = shoot_outputs["soil_meteo"]
        df_meteo_shoot['t'] = df_meteo_shoot.index
        df_meteo_soil['t'] = df_meteo_soil.index
        df_meteo_shoot['day'] = df_meteo_shoot['t'] // 24 + 1
        df_meteo_soil['day'] = df_meteo_soil['t'] // 24 + 1
        av_air_temperature = df_meteo_shoot.groupby(['day'])['air_temperature'].agg('mean')
        av_surface_temperature = df_meteo_shoot.groupby(['day'])['soil_temperature'].agg('mean')
        av_soil_temperature = df_meteo_soil.groupby(['day'])['soil_temperature'].agg('mean')
        daily_sum_PARi = df_meteo_soil.groupby(['day'])['PARi'].agg('sum') * 3600

        dataset['day'] = (np.floor(dataset['t'] / 24) + 1).astype('int64')
        dataset = dataset.set_coords('day')
        mineral_N_concentrations = dataset["soil_Nm"].mean(dim="vid")
        mineral_N_concentrations = mineral_N_concentrations.groupby('day').mean().values
        days = np.unique(dataset["day"].values)

        fig, ax = plt.subplots(3, 1, figsize=figsize)
        ax[0].plot(days, av_air_temperature.to_numpy()[:len(days)], label='Air temperature')
        ax[0].plot(days, av_surface_temperature.to_numpy()[:len(days)], label='Soil surface temperature')
        ax[0].plot(days, av_soil_temperature.to_numpy()[:len(days)], label='Soil temperature')
        ax[1].plot(days, daily_sum_PARi.to_numpy()[:len(days)])
    ...
    def shoot_root_mass_WB(shoot_outputs, shoot_outputs_ref, dataset, outputs_dirpath, thermal_time=True, massic=False):
        ...
        if thermal_time:
            ...
            days_hourly = df_meteo["t"].to_numpy(dtype=float) / 24.0       # 0, 1/24, 2/24, ...
            days_hourly_simu = df_roots["t"].to_numpy(dtype=float) / 24.0
            ...
            time_scale = days_to_tt(days_hourly_simu)
        ...
        if thermal_time:
            ...
            secax.xaxis.set_major_formatter(WB.make_days_formatter(
                17, 12, 2025, fmt="%d/%m", one_based=False
            ))
        ...
        if thermal_time:
            ...
            # 2) NOW create the secondary axis using the *final* TT limits
            secax3 = ax3.secondary_xaxis('bottom', functions=(tt_to_days, days_to_tt))
            secax3.spines['bottom'].set_position(('outward', 35))
            secax3.set_xlabel('Date')
            secax3.xaxis.set_major_formatter(WB.make_days_formatter(
                17, 12, 2025, fmt="%d/%m", one_based=False
            ))
            # secax2.xaxis.set_major_locator(MaxNLocator(integer=True))
    ...
    def XY_totals_all_times_plus_shoot(shoot_outputs, dataset, to_xunit, to_yunit, outputs_dirpath, x=None, y=None, xshoot=None, massic=False, daily_average=True, custom_suffix=""):
        ...
        if massic:
            ...
            y_unit += "/cm2"
            # mass = dataset["living_struct_mass"].sum(dim="vid") * 1000
            mass = dataset["root_exchange_surface"].sum(dim="vid") * 10000 # cm2
            if xshoot is None:
                dsx_all /= mass
            dsy_all /= mass
        ...
        for t, time in enumerate(target_times):
            ...
            for r in range(len(target_roots)):
                ...
                for v in range(len(variables)):
                    ax[v][r].plot(current_root["distance_from_tip"].values * 100, current_root_window[variables[v]].mean(dim='t').values * conversions[v], label=f"day {time // 24}", c=list(twenty_palette.values())[t%len(twenty_palette)])
                    if xlim is not None:
    ...
    def make_days_formatter(start_day=17, start_month=12, start_year=2025, fmt="%d/%m", one_based=False):
        start_dt = datetime(start_year, start_month, start_day)
        def _fmt(x, pos=None):
            base = x - 1.0 if one_based else x
            try:
                d = start_dt + timedelta(days=float(base))
            except Exception:
                return ""
...
class XarrayPlotting:
    ...
    def index_axes(dataset):
        print("Reindexing root segments...")
